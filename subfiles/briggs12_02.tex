...
%\relscale{1.4}
  ...
  \begin{ex*}[\textcolor{blue}{LC 33.8-33.9}]
    Consider the polar curve $r=2\sin(\theta)-1$
  \end{ex*}
  \begin{tasks}[after-item-skip=\stretch{1}, label=,item-indent=0pt](1)
    \task Complete the table below
      \begin{center}
        \begin{tabularx}{0.9\linewidth}{@{}
          >{\hsize=1.54\hsize}X
          |*{6}{>{\hsize=0.82\hsize}Y}}
          $\theta$& $0$& $\pi/6$& $\pi/4$& $\pi/2$& $\pi$& $3\pi/2$\\\midrule
          $r=2\sin(\theta)-1$
        \end{tabularx}
      \end{center}
    \task Graph the polar curve $r=2\sin(\theta)-1$
  \end{tasks}
  \begin{center}
